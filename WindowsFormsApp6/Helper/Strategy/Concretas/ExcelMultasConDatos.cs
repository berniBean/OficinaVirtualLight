...
{
    public class ExcelMultasConDatos : AbstractProgress, IStrategyExcel<CListaRequeridosBO>
    {
        private readonly ToolStripProgressBar _progressBar;
        private readonly ToolStripStatusLabel _tsStatus;
        private readonly Label _lblProgress;

        public ExcelMultasConDatos(ToolStripProgressBar progressBar, ToolStripStatusLabel tsStatus, Label lblProgress)
        {
                ...
                tareas = listaRequerimientos.Select(async item =>
                {
                    ...
                    try
                    {
                        if (progress != null)
                        {
                            ...
                            hojaExcel.Cells[i, "H"] = item.Diligencia == "NO LOCALIZADO" ? DateFormatHelper.FechaCorta(item.FechaNotificacion) : default;//NL
                            hojaExcel.Cells[i, "I"] = DateFormatHelper.FechaCorta(item.FechaCitatorio);//Citatorio
                            hojaExcel.Cells[i, "J"] = item.Diligencia == "LOCALIZADO" ? DateFormatHelper.FechaCorta(item.FechaNotificacion) : default;//Localizado
                            hojaExcel.Cells[i, "K"] = DateFormatHelper.FechaCorta(item.FechaPago);//fechapago
                            hojaExcel.Cells[i, "L"] = item.Importe;//importe                            
                            hojaExcel.Cells[i, "N"] = item.Observaciones;//Observaciones
