...
{
    public class CEmisionActualDAL : obtenerEmision
    {
        private readonly string strConn;

        public CEmisionActualDAL() {
            strConn = System.Configuration.ConfigurationManager.AppSettings["K1"];
        }

        public CEmisionActualDAL(string conn) {
            strConn = conn;
        }
        ...
        public ListCEmisionActual  GetPeriodo(string anhoFiscal) {
            try
            {
                ...
                {
                    MySqlCommand OrdenSql = new MySqlCommand("selectEmision", conn)
                    {
